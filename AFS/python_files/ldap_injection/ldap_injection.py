...
def scan_ldap_injection(url):
    with open("/var/www/AFS/python_files/ldap_injection/ldap_vectors.txt", "r", encoding="utf-8") as f:
        ldap_vectors = f.read().splitlines()
...
#
    ...
    for form in forms:
        ...
        for vector in ldap_vectors:
            ...
            if is_vulnerable(res):
                ldap_injection = {}
                ldap_injection['success'] = 'O'
                ldap_injection['payload'] = repr(vector)[1:-1]
                ldap_injection['vuln_url'] = url

                return ldap_injection


    ldap_injection = {}
    ldap_injection['success'] = 'X'
    ldap_injection['payload'] = '-'
    ldap_injection['vuln_url'] = '-'
    return ldap_injection
